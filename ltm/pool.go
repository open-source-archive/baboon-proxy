...
func ShowLTMPoolMember(host, pool string) *PoolMembers {
...
func PostLTMPool(host string, json *CreatePool) (*backend.Response, error) {
	u, _ := url.Parse(host)
	u.Path = path.Join(u.Path, "pool")
